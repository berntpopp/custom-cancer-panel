...
script_path <- "/analyses/G01_PanelApp/"
...
# filter for cancer related panels
filter_string <- "[Cc]ancer|[Tt]umour|[Mm]Melanoma|[Sc]arcoma|[Pp]olyposis|[Pp]haeochromocytoma|[Pp]araganglioma|[Mm]alignancies|[Gg]enodermatos|[Rr]habdomyosarcoma"

panelapp_panels <- bind_rows(panelapp_uk, panelapp_australia) %>%
  mutate(cancer_disease = str_detect(name, filter_string))

panelapp_panels_cancer <- panelapp_panels %>%
  filter(cancer_disease == TRUE)
...
############################################
## load all cancer related PanelApp panels into one tibble

# TODO: download API call data for reproducibility

panelapp_genes <- panelapp_panels_cancer %>%
  rowwise() %>%
...
write_csv(panelapp_panels_cancer,
  file = paste0("results/G01_PanelApp_panels.",
    creation_date,
    ".csv"),
  na = "NULL")

write_csv(all_panelapp_genes_format,
  file = paste0("results/G01_PanelApp_genes.",
    creation_date,
    ".csv"),
  na = "NULL")

gzip(paste0("results/G01_PanelApp_genes.", creation_date, ".csv"),
  overwrite = TRUE)
############################################
